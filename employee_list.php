...
$searchDesignation = isset($_GET['search_designation']) && $_GET['search_designation'] !== '' ? mysqli_real_escape_string($conn, $_GET['search_designation']) : '';
...
$query = "SELECT employees.eid, PNO, eName, eFHName, eDesigID, designations.DesigName AS DesigName, eCircOffi, circlesoffices.COName AS COName, CNIC, ePhonCell, Gender, eServiceNature FROM employees LEFT JOIN designations ON employees.eDesigID = designations.DID LEFT JOIN circlesoffices ON employees.eCircOffi = circlesoffices.COID";
...
if ($searchDesignation !== '') {
    $conditions[] = "employees.eDesigID = '$searchDesignation'";
}
...
$conditions = apply_employee_area_access($conn, $conditions);
...
$activeQuery = mysqli_query($conn, "SELECT COUNT(*) as count FROM employees WHERE eServiceNature = 1" . ($canAccessAllOffices ?? true ? '' : (empty($allowedOfficeAreas) ? " AND 1 = 0" : " AND eCircOffi IN (" . implode(',', $allowedOfficeAreas) . ")")));
...
<body>
                            ...
                            <form method="GET" class="row g-3">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-select" id="search_designation" name="search_designation">
                                        ...
                                        <?php
                                        $desigQuery = mysqli_query($conn, "SELECT DID, DesigName, DesigBS FROM designations WHERE DPO = 1 ORDER BY DesigName");
                                        while ($desig = mysqli_fetch_assoc($desigQuery)) {
                                            $selected = ($searchDesignation == $desig['DID']) ? 'selected' : '';
                                            $desigLabel = htmlspecialchars($desig['DesigName']);
                                            if (!empty($desig['DesigBS'])) {
                                                $desigLabel .= ' (BS-' . htmlspecialchars($desig['DesigBS']) . ')';
                                            }
                                            echo "<option value='" . $desig['DID'] . "' $selected>" . $desigLabel . "</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-select" id="search_circle" name="search_circle">
                                        ...
                                        if ($canAccessAllOffices) {
                                            ...
                                            $circleQuery = mysqli_query($conn, "SELECT COID, COName FROM circlesoffices WHERE COPO = '1' AND COID IN ($areaList) ORDER BY COName");
                                        } else {
                                            $circleQuery = false;
                                        }
                                        if ($circleQuery) {
                                    ...
                                    <tbody>
                                        ...
                                        if ($result && mysqli_num_rows($result) > 0) {
                                            while ($row = mysqli_fetch_assoc($result)) {
                                                ...
                                                $desigDisplay = (isset($row['DesigName']) && $row['DesigName']) ? $row['DesigName'] : $row['eDesigID'];
                                                echo "<td>" . htmlspecialchars($desigDisplay) . "</td>";
                                                ...
                                                echo "<a href='" . append_auth_token('edit_employee.php?id=' . $row['eid']) . "' class='btn btn-sm btn-warning' title='Edit' style='flex-shrink: 0;'><i class='fas fa-edit'></i></a>";
    ...
    <script>
        ...
        function viewEmployee(employeeId) {
            ...
            fetch('<?php echo append_auth_token('get_employee_detail.php'); ?>&id=' + employeeId)
        ...
        function toggleStatus(employeeId, currentStatus) {
            ...
            if (confirm('Are you sure you want to ' + actionText + ' this employee?')) {
                fetch('<?php echo append_auth_token('toggle_employee_status.php'); ?>', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                        },
                        body: 'id=' + employeeId + '&status=' + newStatus + '&auth_token=' + encodeURIComponent('<?php echo $_SESSION['auth_uuid'] ?? ''; ?>')
                    })
        ...
        function deleteEmployee(employeeId) {
            if (confirm('Are you sure you want to delete this employee record? This action cannot be undone.')) {
                fetch('<?php echo append_auth_token('delete_employee.php'); ?>', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                        },
                        body: 'id=' + employeeId + '&auth_token=' + encodeURIComponent('<?php echo $_SESSION['auth_uuid'] ?? ''; ?>')
                    })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
